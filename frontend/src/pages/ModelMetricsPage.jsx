...
import { Database, Filter, Layers, Settings2, BarChart2, Star, ExternalLink, Activity } from 'lucide-react';
...
export default function ModelMetricsPage() {
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in-up pb-10 h-full">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-white/10 pb-4 gap-4">
        ...
        {/* Navigation Tabs */}
        <div className="flex bg-[#0F172A] p-1 rounded-lg border border-[rgba(255,255,255,0.05)]">
           <button 
           ...
           <button 
             ...
             className={`px-4 py-2 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${
               activeTab === 'mlflow' 
                 ? 'bg-[#0194E2] text-white shadow-lg' 
                 : 'text-gray-400 hover:text-white hover:bg-white/5'
             }`}
           >
             <Activity size={16} />
      ...
      {activeTab === 'mlflow' && (
        <div className="flex-1 flex flex-col h-[800px] glass-card overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2 border-b border-white/10 bg-[#0F172A]">
            <span className="text-xs text-gray-400 font-semibold uppercase tracking-wider">Embed: MLflow Tracking Server (localhost:5050)</span>
            <a href="http://localhost:5050" target="_blank" rel="noreferrer" className="text-xs flex items-center gap-1 text-blue-400 hover:text-blue-300">
              Open config in new tab <ExternalLink size={12} />
            </a>
          </div>
          <iframe 
